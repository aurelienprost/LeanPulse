...
set LIBDIR=
if "%OS%"=="Windows_NT" set LIBDIR=%~dp0
...
set JAVAOPTS=-Xmx1024m -Denv.windir=%WINDIR% -Djavax.xml.transform.TransformerFactory=com.leanpulse.syd.remote.internal.CachingTransformerFactory
...
:runFop
rem ECHO "%JAVACMD%" %JAVAOPTS% %LOGCHOICE% %LOGLEVEL% -cp "%LIBDIR%syd-rmrender.jar" %FOP_OPTS% org.apache.fop.cli.Main %FOP_CMD_LINE_ARGS%
"%JAVACMD%" %JAVAOPTS% %LOGCHOICE% %LOGLEVEL% -cp "%LIBDIR%syd-rmrender.jar" %FOP_OPTS% org.apache.fop.cli.Main %FOP_CMD_LINE_ARGS%
