...
<xsl:stylesheet xmlns:fo="http://www.w3.org/1999/XSL/Format"
	...
	xmlns:dt="uuid:C2F41010-65B3-11d1-A29F-00AA00C14882" version="2.0">
	...
	<xsl:template name="ApplyParaProperties">
		...
		<!-- style-level (pStyle) -->
		<xsl:apply-templates select="key('styles', w:pPr/w:pStyle/@w:val, ancestor::w:wordDocument)[@w:type='paragraph']" />
		<!-- handle 'keepNext' for table cells specifically -->
		<xsl:if test="parent::w:tc and ../w:p[1]/w:pPr/w:keepNext[not(attribute::*) or @val = 'on']">
	...
	<!-- Represents Widow/Orphan Control option: prevents Word from printing the last line of -->
	<!-- a paragraph by itself at the top of the page (widow) or the first line of a paragraph at -->
	...
	<!-- Parent elements: w:pPr -->
	<!-- 2) the amount by which the spacing between characters is expanded or condensed. -->
	...
	<xsl:template match="w:spacing">
		...
		<xsl:if test="@w:before-lines and not($mode='after')">
			<xsl:attribute name="space-before"><xsl:value-of
				select="@w:before-lines *.12" /><xsl:text>pt</xsl:text></xsl:attribute>
			<xsl:if test="not(@w:before-autospacing) or @w:before-autospacing='off'">
				<xsl:attribute name="space-before.conditionality"><xsl:text>retain</xsl:text></xsl:attribute>
			</xsl:if>
		</xsl:if>
		...
		<xsl:if test="@w:line and not($mode='before') and not($mode='after')">
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:attribute name="line-height">
                <xsl:value-of
						select="((@w:line div 20) div 12) * $default-line-height" />
                <!--<xsl:text>pt</xsl:text>-->
              </xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template match="w:ind">
		<!-- if mode='text-indent' - only attribute @text-indent need to be written -->
		<xsl:param name="mode" />
		<!-- start-indent/end-indent -->
		<xsl:if test="not($mode) or $mode='left'">
			<xsl:if test="@w:left">
		...
		<xsl:if test="not($mode) or $mode='right'">
			<xsl:if test="@w:right">
			...
			<xsl:if test="@w:right-chars">
				<xsl:attribute name="end-indent"><xsl:value-of
					select="@w:right-chars *.12" /><xsl:text>pt</xsl:text></xsl:attribute>
			</xsl:if>
		</xsl:if>
		<!-- text-indent -->
		<xsl:if test="@w:first-line">
			<xsl:attribute name="text-indent"><xsl:value-of
				select="@w:first-line div 20" /><xsl:text>pt</xsl:text></xsl:attribute>
		</xsl:if>
		<xsl:if test="@w:first-line-chars">
			<xsl:attribute name="text-indent"><xsl:value-of
				select="@w:first-line-chars *.12" /><xsl:text>pt</xsl:text></xsl:attribute>
		</xsl:if>
		...
		<xsl:if test="@w:hanging-chars">
			<xsl:attribute name="text-indent"><xsl:value-of
				select="- @w:hanging-chars *.12" /><xsl:text>pt</xsl:text></xsl:attribute>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="ApplyRunProperties">
		<xsl:variable name="runStyle"
			select="key('styles', w:rPr/w:rStyle/@w:val, ancestor::w:wordDocument)[@w:type='character']/w:rPr/*" />
		<xsl:variable name="paraStyleR"
			select="key('styles', ancestor::p/w:pPr/w:pStyle/@w:val, ancestor::w:wordDocument)[@w:type='paragraph']/w:rPr/*" />
		<!-- default style -->
		<xsl:apply-templates select="$default-character-style" />
		<!-- style-level -->
		<xsl:for-each select="$runStyle">
			<xsl:if
				test="not($paraStyleR[name()=name(current()) and current()/@w:val='off'])">
				<xsl:apply-templates select="." />
			</xsl:if>
		</xsl:for-each>
	...
	<!-- Formats lowercase text as capital letters and reduces their size -->
	<!-- Parent elements: w:rPr -->
	...
	<!-- Adjusts the vertical position of the text relative to the baseline and changes the font size if possible. -->
	<!-- To raise or lower the text without reducing  the font size, use the position element. -->
	<!-- Parent elements: w:rPr -->
	...
	<xsl:template match="w:color">
		<!-- When w:color is specified under w:p/w:pPr/w:rPr element, it's incorrect 
			to apply color attribute for all runs of the paragraph. Thus color attribute 
			applies for a run only when w:color is explicitly specified. -->
	...
	<xsl:template match="w:style">
		...
		<xsl:apply-templates
			select="key('styles', w:basedOn/@w:val, ancestor::w:wordDocument)">
		...
		<xsl:apply-templates
			select="w:tblStylePr/*[not(self::w:trPr) and not(self::w:tcPr)]" />
	</xsl:template>
	...
	<!-- ====================================== -->
	<!-- Style-level properties for table-cells -->
	<!-- ====================================== -->
	...
	<xsl:template match="w:pPr">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates
					select="*[not(self::w:pStyle) and not(self::w:listPr)]" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="w:tblPr">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates
					select="*[not(self::w:tblStyle) and not(self::w:tblCellMar)]" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<!-- ============================ -->
	<!-- Disallows breaks within a table row -->
	<!-- Parent element: w:trPr. -->
	<!-- ============================ -->
	<xsl:template match="w:cantSplit">
		<xsl:choose>
			...
			<xsl:when test="./@val = 'off'">
				<xsl:attribute name="keep-together.within-page">never</xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="keep-together.within-page">always</xsl:attribute>
			</xsl:otherwise>
	...
	<xsl:template match="w:insideV">
		<xsl:param name="cell" />
		<xsl:variable name="specifier-node" select="." />
		<xsl:call-template name="PutBorderSpecificsForCell">
			<xsl:with-param name="specifier-node" select="$specifier-node" />
			<xsl:with-param name="cell" select="$cell" />
			<xsl:with-param name="direction" select="'vertical'" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="w:insideH">
		...
		<xsl:call-template name="PutBorderSpecificsForCell">
			<xsl:with-param name="specifier-node" select="$specifier-node" />
			<xsl:with-param name="cell" select="$cell" />
			<xsl:with-param name="direction" select="'horizontal'" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="w:hmerge">
		<xsl:if test="@w:val='restart'">
			<xsl:call-template name="CountCellsSpanned">
				...
				<xsl:with-param name="following-cell"
					select="ancestor::w:tc[1]/following-sibling::w:tc[1]" />
	...
	<xsl:template match="w:vmerge">
		<xsl:if test="@w:val='restart'">
			<xsl:variable name="column-number"
				select="count(.|ancestor::w:tc[1]/preceding-sibling::w:tc[not(w:tcPr/w:gridSpan)]) + sum(ancestor::w:tc[1]/preceding-sibling::w:tc/w:tcPr/w:gridSpan/@w:val)" />
			<xsl:call-template name="CountCellsSpanned">
				<xsl:with-param name="number-cells-spanned" select="1" />
				<xsl:with-param name="following-cell"
					select="ancestor::w:tr[1]/following-sibling::w:tr[1]/w:tc[count(.|preceding-sibling::w:tc[not(w:tcPr/w:gridSpan)]) + sum(preceding-sibling::w:tc/w:tcPr/w:gridSpan/@w:val) = $column-number]" />
	...
	<!-- Parent elements: w:divBdr, w:pgBorders, w:pBdr, w:tblBorders, w:tcBorders -->
	<xsl:template match="w:top|w:bottom|w:left|w:right">
		<!--<xsl:param name="cell"/> -->
		<xsl:param name="cell" select="/.." /> <!-- Initialize param value as empty node-set -->
		<xsl:variable name="specifier-node" select="." />
		<xsl:call-template name="PutBorderSpecificsForCell">
			<xsl:with-param name="specifier-node" select="$specifier-node" />
			<xsl:with-param name="cell" select="$cell" />
			<xsl:with-param name="direction" select="local-name()" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template
		match="w10:bordertop|w10:borderleft|w10:borderbottom|w10:borderright">
		<xsl:variable name="style-attribute-name"
			select="concat('border-', substring-after(local-name(), 'border'), '-style')" />
		...
		<xsl:if test="@width">
			<xsl:attribute
				name="{concat('border-',substring-after(local-name(), 'border'),'-width')}">
	...
	<xsl:template
		match="@o:bordertopcolor|@o:borderleftcolor|@o:borderbottomcolor|@o:borderrightcolor">
		<xsl:attribute
			name="{concat('border-', substring-before(substring-after(local-name(), 'border'), 'color'), '-color')}">
          <xsl:choose>
            <xsl:when test=".='this'"><xsl:text>black</xsl:text></xsl:when>
	...
	<xsl:template match="wx:shd">
		<xsl:attribute name="background-color">
          <xsl:call-template name="ConvertHexColor">
            ...
            <xsl:with-param name="type" select="'bgcolor'" />
          </xsl:call-template>
        </xsl:attribute>
	</xsl:template>

	<!-- Represents shading and background-color. -->
	<!-- Parent elements: w:pPr, w:rPr, w:tblPr, w:tblPrEx, w:tcpr. -->
	<xsl:template match="w:shd">
		<xsl:if test="@w:fill">
			<xsl:attribute name="background-color">
            <xsl:call-template name="ConvertHexColor">
              <xsl:with-param name="hexColor" select="@w:fill" />
              <xsl:with-param name="type" select="'fill'" />
            </xsl:call-template>
          </xsl:attribute>
		</xsl:if>
		<xsl:if test="@w:color and not(../w:color)">
			<xsl:attribute name="color">
            <xsl:call-template name="ConvertHexColor">
              <xsl:with-param name="hexColor" select="@w:color" />
              <xsl:with-param name="type" select="'color'" />
              <xsl:with-param name="bgColor" select="@w:fill" />
            </xsl:call-template>
          </xsl:attribute>
		</xsl:if>
	</xsl:template>
